    ...
    <section className="mb-6 rounded-2xl border border-stone-800 bg-stone-950/50 p-4 md:p-5">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="h-8 rounded-md border border-stone-700 px-3 text-xs text-stone-400 disabled:opacity-50"
        >
          {loading ? "..." : "새로고침"}
        </button>
      </div>
      ...
      <div className="space-y-2">
        {candidates.slice(0, 5).map((candidate) => (
          <article
            key={candidate.id}
            className="rounded-xl border border-stone-800 bg-stone-900/35 p-3"
          >
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              ...
              <div className="flex shrink-0 gap-2">
                <button
                  ...
                  className="h-8 rounded-md border border-amber-300/25 bg-amber-300/10 px-3 text-xs text-amber-200 disabled:opacity-50"
                >
                  적용
                </button>
                <button
...
function PolicyBadge({ kind }: { kind: PolicyKind }) {
  const meta = {
    ALLOW_AFTER_SUGGESTION: ["더 자신 있게", "border-emerald-400/20 bg-emerald-400/10 text-emerald-300"],
